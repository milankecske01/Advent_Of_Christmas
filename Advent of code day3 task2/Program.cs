...
for (int i = 0; i < input.Length; i++)
{
    if (input[i]=='*')
    {
        //Console.WriteLine($"Found gear: {i}");
        List<int> adjacentNumbers = AdjacentNumbers(i);
        if (adjacentNumbers.Count==2)
        {
            Console.WriteLine($"Found gear: {adjacentNumbers[0]}*{adjacentNumbers[1]}");
...
//bug amire érdemes előre készülni!
//ha pont fölötte/alatta van egy szám akkor azt duplán fogja bevenni pl:
//123
//.*.
//...
...
List<int> AdjacentNumbers(int index)
{
    ...
    bool leftside = IsLeftSide(index);
    bool rightside = IsRightSide(index);
    if (index > width)//top
    {
        if (!leftside && IsNumber(input[index - width - 1]))//top left diagonal
        {
            numberIndexes.Add(GetNumber(index - width - 1));
        }
        if (!rightside && IsNumber(input[index - width + 1]))//top right diagonal
        {
            numberIndexes.Add(GetNumber(index - width + 1));
        }
        if (IsNumber(input[index-width]))
        {
            numberIndexes.Add(GetNumber(index - width));//top
        }

    }
    if (!leftside && IsNumber(input[index - 1]))//left
    {
        numberIndexes.Add(GetNumber(index - 1));
    }
    if (!rightside && IsNumber(input[index + 1]))//right
    {
        numberIndexes.Add(GetNumber(index + 1));
    }
    if (index < input.Length - width)
    {
        if (!leftside && IsNumber(input[index + width - 1]))//bottom left diagonal
        {
            numberIndexes.Add(GetNumber(index + width - 1));
        }
        if (!rightside && IsNumber(input[index + width + 1]))//bottom right diagonal
        {
            numberIndexes.Add(GetNumber(index + width + 1));
        }
        if (IsNumber(input[index + width]))
        {
            numberIndexes.Add(GetNumber(index + width));//bottom
        }
    }
    //alatta felesleges megnézni mivel mindem szám min 2 hosszú
    int[] prev = numberIndexes[0];
    for (int i = 1; i < numberIndexes.Count; i++)
    {
        if (prev[0] == numberIndexes[i][0]&& prev[1] == numberIndexes[i][1])
        {
            numberIndexes.RemoveAt(i);
            i--;
        }
        prev = numberIndexes[i];
    }
    foreach (var number in numberIndexes)
    {
        output.Add(int.Parse(input.Substring(number[0], number[1] - number[0]+1)));
    }
    return output;
    ...
    int[] GetNumber(int index)/*return: int[] {startindex,endindex}*/
    {
       ...
       // Console.WriteLine($"found number: {startindex}-{endindex}");
        return new int[] { startindex, endindex };
    }
    bool IsLeftSide(int index)
    {
        return index % (width) == 0 ? true : false;
    }
    bool IsRightSide(int index)
    {
        return (index + 1) % (width) == 0 ? true : false;
    }
}